...
% This version is for the linearity analysis (called by
% 'Sim_1layer_cluster_sameParams.m'):
% p_stim.NSource = 2 (line 137)
% p_stim.Rx = [0.003 0.007] if we want half the field stimulated with 
% Poisson spiking at 3 Hz and the other half at 7 Hz; modify on line 140
% based on needs and update the filename in 
% 'Sim_1layer_cluster_sameParams.m' accordingly.

function RecFeed2D_linear(varargin)
...
% Number of sources for global correlation
p_stim.Nsource = 2;  

% Rate of neurons in feedforward layer (kHz), size 1xNsource
p_stim.rX = [0.01, 0.01];
