...
/* sx is the feedforward presynaptic spike trains, which should be 2xNsx where Nsx is the number of spikes.
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    for(isyn=0;isyn<m1*m2; isyn++){
        if(Psyn[isyn]){
            ...
            temp1[Nsyn]=(1/taudsyn[isyn]+1/taursyn[isyn]);
    ...
    /* (OG): Vleak modified to be different for each neuron */
    ...
    if(m1==1 && m2!=1)
        m1=m2;
    if(m1!=N) /* (OG): make sure that V0 and Vleak are of the same size */
        mexErrMsgTxt("Vleak should be of the same size as V0");
        ...
        for(j=0;j<N;j++){
            ...
            if(j<Ne){
                ...
                if(refstate[j]<=0){
                    Isyntot=0;
                    for(isyn=0;isyn<Nsyn;isyn++){
                        Isyntot+=Psyn2[isyn]*Isyn[j*Nsyn+isyn];
                    }
                    /* (OG) : Vleak[0] replaced by Vleak[j] because it is different for each excitatory neuron */
                    v[j]+=fmax((Isyntot-gl[0]*(v[j]-Vleak[j])+gl[0]*DeltaT[0]*EXP((v[j]-VT[0])/DeltaT[0]))*dt/C[0],Vlb[0]-v[j]);}
                else{
                    if(refstate[j]>1)
                        v[j]=Vth[0];
                    else
                ...
                if(refstate[j]<=0){
                    ...
                    /* (OG) : Vleak[1] replaced by Vleak[j] because it is different for each inhibitory neuron */
                    v[j]+=fmax((Isyntot-gl[1]*(v[j]-Vleak[j])+gl[1]*DeltaT[1]*EXP((v[j]-VT[1])/DeltaT[1]))*dt/C[1],Vlb[1]-v[j]);}
                else{
                    if(refstate[j]>1)
                        v[j]=Vth[1];
                    else
