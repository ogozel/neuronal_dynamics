...
%           1st row contains spike times
%           2nd row contains indices of neurons that spike
% N1, N2: total number of neurons in Pop1 & Pop2, respectively
% Nc: 1x2 number of sampled neurons, e.g. Nc=[500, 500];
...
%      % NB from OG: also saved as 'C' sometimes!
...
switch dim
    ...
    case '2D'
        ...
        % Indices of all the neurons which spiked (excitatory and inhibitory)
        I1=transpose(unique(s1(2,:)));
        I2=transpose(unique(s2(2,:)));
        
        I1=I1(I1<=N1); % only select excitatory neurons
        I2=I2(I2<=N2&I2>0);
        ...
        % Indices of Nc(1) randomly selected excitatory neurons from L1
        Ic1=randsample(I1,Nc(1)); 
        % Indices of Nc(2) randomly selected excitatory neurons from L2
        Ic2=randsample(I2,Nc(2)); 
end
...
% Compute instantaneous firing rate per 1ms time window
re1=zeros(Nc(1),length(time)); % ... of Nc(1) randomly selected excitatory neurons in L1
re2=zeros(Nc(2),length(time)); % ... of Nc(2) randomly selected excitatory neurons in L2

% Get the number of spikes per 1ms timewindow (loop over each
