% This code runs simulations of a 2-layer spiking recurrent neural network
% L0: input layer
% L1: output layer

% I run 30 trials of 20s each for each parameter set (spatial
% destabilization in L1: sigma_I_rec=0.3, or temporal destabilization in 
% L1: tau_I_decay=24ms). All other parameters are identical to the standard
% network parameters.
...
% Determine which Poisson input to use in the associated
% 'RecFeed2D_linear.m' script (see comments at the top of the script) and 
% modify the filename in this script (line 292) accordingly.
...
% Network type
this_networkType = 'RecFeed2D_linear';
...
bool_saveSpkCnts = 0; % if 1, save spike counts from all layers
...
if pid<=1
    ...
    nType=pid;
    sigmaRRfromI_range = [0.3];
    
    disp(['Modify sigmaRRfromI in L1 (Output layer) to ',...
...
else %%% Modify tau_I_decay %%%
    
    nType=pid-1;
    tauIdecay_range = [24];
    
    disp(['Modify tauIdecay in L1 (Output layer) to ',...
...
for trial = nt
    ...
    opt.saveSx = 1; % save spike times from Layer 0 (input layer) (default is 0, save to filename)
    opt.saveS1 = 1; % save spike times from Layer 1 (default is 1, save to filename, or s1_fname if specified)
    opt.saveS2 = 0; % save spike times from Layer 2 (default is 1, save to filename)
    ...
    filename=strrep(sprintf(strcat('%s',this_networkType,'_',this_stimulationType,'_saveSpkCnts',num2str(bool_saveSpkCnts),'_fixW',num2str(bool_fixW),'_%s_10and10_ID%.0f'),...
    ...
    filename
    RecFeed2D_linear(opt, ParamChange) % main simulation
